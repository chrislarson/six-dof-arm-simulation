%% Evaluation Script
function Eval_Script(projectNumber,functionName,N, dontTime)
if ~exist('functionName','var')
    functionName = {'all'};
end

if ~exist('dontTime','var')
    dontTime = false;
end

tc_new = load('Test_Cases.mat');
tc_old = load('Test_Cases_old.mat');
% test_case = [tc_new.test_case,tc_old.test_case];
test_case = [tc_new.test_case];
% test_case = [tc_old.test_case];

fun_list{1} = {'rotX';'rotY';'rotZ';'rpy2Rot';'rot2RPY';'cpMap';'angleAxis2Rot';'rot2AngleAxis';'rot2Quat';'quat2Rot';'twist2Transform';'transform2Twist';'multiplyQuat';'pq2DualQuat';'dualQuat2PQ';'multiplyDualQuat';'dhTransform';};
...
if ~exist('projectNumber','var') || isempty(projectNumber)
    projectNumber = 1:4;
end

for pn = projectNumber
    ...
    for i=1:length(test_case)
        ...
        fprintf(fileID,'\n\n\toutput.percentError = -1*ones(%i,1);\n',fun.Number_Outputs);
        
        
        fprintf(fileID,'\n\n\toutput_has_nan = false;');
        for i=1:fun.Number_Outputs
            fprintf(fileID,['\n\tif all(all(isnumeric(O%i))) && any(any(~isfinite(O%i)))'...
                '\n\t\toutput_has_nan=true;'...
                '\n\t\toutput.percentError(%i) = -1;'...
                '\n\t\toutput.error_text{end+1} = [''Output %i is not finite.''];'...
                '\n\tend\n\n'],i,i,i,i);
        end
        fprintf(fileID,'\n\tif output_has_nan\n\t\treturn;\n\tend\n\n');
